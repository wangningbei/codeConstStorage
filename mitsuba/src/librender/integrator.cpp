...
void SamplingIntegrator::renderBlock(const Scene *scene,
	...
	for (size_t i = 0; i<points.size(); ++i) {
		...
		const int SUB_SAMPLE_NUM = sqrt(sampler->getSampleCount());
		for (int p = 0; p < SUB_SAMPLE_NUM; p++) {
			for (int q = 0; q < SUB_SAMPLE_NUM; q++) {
				rRec.newQuery(queryType, sensor->getMedium());
				//                Point2 samplePos(Point2(offset) + Vector2(1.0 / SUB_SAMPLE_NUM * (p + 0.5), 1.0 / SUB_SAMPLE_NUM * (q + 0.5)));
				Point2 samplePos(Point2(offset) + Vector2(1.0 / SUB_SAMPLE_NUM * (p + sampler->next1D()), 1.0 / SUB_SAMPLE_NUM * (q + sampler->next1D())));
				...
				// printf("%d %d\n", rRec.pixelIndex.x, rRec.pixelIndex.y);
