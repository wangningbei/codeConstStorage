...
public:
	...
	Spectrum Li(const RayDifferential &r, RadianceQueryRecord &rRec) const {
		...
		while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
			...
			Float randNum = rRec.nextSample1D();
...
#if 0
						const Float MAX_INDIRECT_CONTRIBUTION = 10.0f;
						if (rRec.depth >= 2) {
							Float r, g, b;
							result.toLinearRGB(r, g, b);
							r = fmin(r, MAX_INDIRECT_CONTRIBUTION);
							g = fmin(g, MAX_INDIRECT_CONTRIBUTION);
							b = fmin(b, MAX_INDIRECT_CONTRIBUTION);
